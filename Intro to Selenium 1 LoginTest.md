...
- **Java 17 or later**; You can verify by running `java -version` in a terminal. If Java is not installed, download and install it now.   
- **Gradle**; You can verify by running `gradle --version`. If Gradle is not installed, follow the instructions at [https://gradle.org/install/](https://gradle.org/install/). On macOS you can use `brew install gradle`; on Windows, the Gradle website provides an installer.  
- **Git**; You can verify by running `git --version`. If Git is not installed, download it from [https://git-scm.com/downloads](https://git-scm.com/downloads).  
- **Chrome browser**; Selenium will automate this browser. Any recent version is fine.  
- **A GitHub account**; If you don't have one, create a free account at [https://github.com](https://github.com).  
- **An IDE**; Eclipse, IntelliJ IDEA Community Edition (free) or VS Code with the Java Extension Pack. 

### 2.2 Create the GitHub Repository

1. Go to [https://github.com](https://github.com) and sign in.  
2. Click the **New** button and select **New repository**.  
...
8. Select a license, if you want.   
...
| cd \~/projectsgit clone https://github.com/YOUR-USERNAME/intro\-to\-selenium.gitcd intro\-to\-selenium |
| :---- |
...
Edit .gitignore and make sure it has these entries:  
.gitattributes  
.idea/  
.project  
.gradle  
...
1. **Continue?:** yes (because files are in this directory)  
2. **Project type:** `application` (Note: we won't use the main application class, but this generates the full project structure we need including the test directory)  
3. **Implementation language:** `Java`  
4. **Target Java version:** `(Take the default)`  
5. **Project name:** `(Take the default)`  
6. **Application structure:** `Single application`  
7. **Build script DSL:** `Groovy`  
8. **Test framework:** `JUnit Jupiter`  
9. For any other prompts (e.g. version of Java, generating multiple subprojects), accept the defaults.

Move the app/src directory to the current directory, and then delete the app directory.

| 	\# linux or mac pro	mv app/\* .	rm \-rf app	\# windows	xcopy \-sve app .	rmdir app |
| :---- |
...
You should see `BUILD SUCCESSFUL`. If so, the project is set up correctly.
...
Add a test section so that test results can be viewed (otherwise gradle will suppress test output):

| test {    testLogging {        showStandardStreams \= true    }} |
| :---- |

The project requires two new dependencies, for Selenium and the WebDriverManager. Add them to the dependencies section:  
...
**WebDriverManager automates this entire process.** When you call  
...
**JUnit Jupiter** is already included in the build. It refers to JUnit 5, the test framework. It provides the `@Test` annotation, assertion methods like `assertEquals`, and the test lifecycle annotations (`@BeforeEach`, `@AfterEach`, etc.). The call to `useJUnitPlatform()` should already be in the test block, so that Gradle knows to use JUnit 5\.

Run the test again to make sure the new dependencies can be downloaded.  
./gradlew test 
...
Write a JUnit 5 test method that verifies a successful login to [saucedemo.com](http://saucedemo.com).  
But first, login to saucedemo manually from the browser, to better understand what the code will need to do.
...
**Steps to perform in the test method:**
...
- You can run the test inside the IDE or from the command line. In practice, these tests would be run from the command line.
...
Write a second test method **testLockedOut**() in the same `LoginTest` class.
...
| WebDriverManager documentation | [https://bonigarcia.dev/webdrivermanager/](https://bonigarcia.dev/webdrivermanager/) |
...
| GitHub Getting Started | [https://docs.github.com/en/get-started](https://docs.github.com/en/get-started) |
| Gradle User Guide | [https://docs.gradle.org/current/userguide/userguide.html](https://docs.gradle.org/current/userguide/userguide.html) |
...
- If you are prompted for credentials, you may need to set up a GitHub personal access token or SSH key. See [https://docs.github.com/en/authentication](https://docs.github.com/en/authentication) for instructions.
...
- Alternatively, you can download the Gradle binary distribution from [https://gradle.org/releases/](https://gradle.org/releases/) and add its `bin/` directory to your PATH.
